...
def ingest_data(days_back: int = 7) -> dict:
    ...
    # â”€â”€ 1e. Weather history + forecast â”€â”€
    logger.info("=" * 60)
    logger.info("INGEST: Weather history + forecast")
    logger.info("=" * 60)
    try:
        weather_provider = os.getenv("PFC_WEATHER_PROVIDER", "open_meteo").strip().lower()
        if weather_provider == "dwd":
            from pfc_shaping.data.ingest_weather_dwd import fetch_and_cache as fetch_weather

            weather_df = fetch_weather()
            status["weather"] = f"OK provider=dwd ({len(weather_df)} rows)"
        else:
            ...
            status["weather"] = f"OK provider=open_meteo ({len(weather_df)} rows)"
