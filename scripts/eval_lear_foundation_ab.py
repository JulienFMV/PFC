#!/usr/bin/env python3
"""Reproducible A/B harness for LEAR foundation blend evaluation."""
...
def _run_variant(
    ...
    foundation_blend_max_horizon_days: int,
    ...
    bt["variant"] = label
    meta = {
        "fm_model": getattr(model._fm, "CHRONOS2_MODEL", None) if model._fm is not None else None,
        "fm_backend": getattr(model._fm, "backend", None) if model._fm is not None else None,
        "fm_device": getattr(model._fm, "_device", None) if model._fm is not None else None,
        "foundation_blend_max_horizon_days": int(foundation_blend_max_horizon_days),
    }
    return bt, meta


def main() -> None:
    parser = argparse.ArgumentParser(description="Run reproducible LEAR foundation A/B.")
    ...
    parser.add_argument("--foundation-max-horizon", type=int, default=1)
    args = parser.parse_args()

    _set_seed(args.seed)
    run_id = datetime.now(timezone.utc).strftime("lear_foundation_ab_%Y%m%d_%H%M%S_%f")
    ...
    experiment, experiment_meta = _run_variant(
        "foundation",
        True,
        args.foundation_max_horizon,
        epex_ch,
        epex_de,
        entso,
        args.n_days,
    ...
    experiment_path = args.output_dir / f"{run_id}_foundation.parquet"
    ...
    payload = {
        **manifest,
        ...
        "foundation_backtest": str(experiment_path),
        "baseline_meta": baseline_meta,
        "foundation_meta": experiment_meta,
        "baseline": baseline_score,
        "foundation": experiment_score,
        "delta": {
            k: experiment_score[k] - baseline_score[k]
            for k in ["mae", "rmse", "mape", "corr"]
        },
        "baseline_breakdown": _segment_metrics(baseline),
        "foundation_breakdown": _segment_metrics(experiment),
    }
