...
class LEARForecaster:
    ...
    def _fit_lasso_for_hour(
        ...
        for window in self.CALIBRATION_WINDOWS:
            ...
            try:
                ...
                model = ElasticNetCV(l1_ratio=0.5, max_iter=self.max_iter, cv=5)
    ...
    def _recalibrate_variance(
        ...
        recalibrated = float(np.clip(recalibrated, actual_mean - 1.2 * actual_std, actual_mean + 1.2 * actual_std))
            ...
            for hour in range(24):
                ...
                if n_rec >= 10 and hour_std > 1.0:
                    ...
                    if lasso_s > 1e-6:
                        ...
                        forecast = float(np.clip(forecast, hour_mean - 1.2 * hour_std, hour_mean + 1.2 * hour_std))
                    else:
