...
| 5-03-02 | 03 | 3 | NEG-04 | — | `fit_peak_ratios` emits `DeprecationWarning` and delegates to `fit_peak_spreads` (transparent shim per RESEARCH §Pattern 6, overriding CONTEXT.md D-A4-2 NotImplementedError approach after callsite audit confirmed both production callers have `spot_history` available) | unit | `pytest tests/test_phase05_negative_prices.py::test_fit_peak_ratios_deprecated -x` | ❌ W0 | ⬜ pending |
