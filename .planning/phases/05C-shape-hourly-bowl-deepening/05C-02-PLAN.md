...
must_haves:
  ...
  artifacts:
    ...
    - path: "tests/test_shape_hourly_bowl.py"
      provides: "Two new tests: `test_split_level_anomaly_invariant` (D-A4-4), `test_f_H_amplitude_preserved_at_M30` (D-A4-6). Updated `SC3_M30_AMPLITUDE_THRESHOLD` constant (calibrated in this plan's Wave 0)."
      contains: "test_split_level_anomaly_invariant"
  key_links:
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    1. Module docstring with sections: Purpose (one line), Math (the two equations from D-A2-2: `level[t] = mean_h(f_H | saison(t), type_jour(t))`, `anomaly[t] = f_H[t] - level[t]`), Invariants (ulp-exact sum, zero-mean per cell), Args (types + index expectations), Returns (`(level, anomaly)` both pd.Series with same index as `f_H_series` and names `"level"` / `"anomaly"`).
  ...
  <done>`_split_level_anomaly` helper exists at module level, exposed via `__all__`, passes the two D-A2-2 invariants on the python-line sanity check. No downstream consumer yet.</done>
</task>
...
<task type="auto">
  ...
  <done>Assembler integration complete. Flag=OFF still produces byte-identical math; flag=ON now executes the split-based damping with telemetry. Full suite green.</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    - tests/test_shape_hourly_bowl.py (Plan 05C-01 state — placeholder `SC3_M30_AMPLITUDE_THRESHOLD = 0.50`)
  ...
  <action>
    Run a one-shot calibration script inside the implementation session (do NOT commit the script — commit only its output value into `tests/test_shape_hourly_bowl.py`).

    Calibration protocol (executed by the implementer in their terminal):
    1. Build the bowl fixture: `epex_df, hydro_df = build_bowl_fixture(seed=42)` and `cal_3yr = enrich_15min_index(epex_df.index, country="CH")`.
    2. Fit both `sh_off = ShapeHourly(use_seasonal_hourly=False).fit(epex_df, cal_3yr, hydro_df)` and `sh_on = ShapeHourly(use_seasonal_hourly=True).fit(epex_df, cal_3yr, hydro_df)`.
    3. Fit a minimal ShapeIntraday on the same data (mirror the pattern in `tests/fixtures/_generate_baseline.py:128-133`).
    4. Build two PFCs at horizon ~M+30 (start_date sufficiently far ahead so months_ahead ~= 30): `start_date='2029-06-01'`, `horizon_days=31`, `reference_date=pd.Timestamp('2027-01-01', tz='UTC')` (gives months_ahead ~= 29-30 across the 31-day window).
    5. For each PFC `df_off`/`df_on`, compute `ptp_off = float(np.ptp(df_off['f_H']))` and `ptp_on = float(np.ptp(df_on['f_H']))`.
    6. Compute the threshold per RESEARCH §M+30: `threshold = max(ptp_on - 0.20, ptp_off * 1.50, 0.50)`. Round to 4 decimals.
    7. Print all values: `print(f"ptp_off={ptp_off:.4f} ptp_on={ptp_on:.4f} threshold={threshold:.4f}")`.

    Commit the measured `threshold` into `tests/test_shape_hourly_bowl.py` by replacing the placeholder line `SC3_M30_AMPLITUDE_THRESHOLD = 0.50  # TODO Plan 05C-02 Wave 0: ...` with `SC3_M30_AMPLITUDE_THRESHOLD = <measured>  # Wave 0 calibrated on bowl_seed42 by Plan 05C-02 (RESEARCH §Lever 2 measure-then-assert). Observed: ptp_off=X.XXXX, ptp_on=Y.YYYY. Formula: max(ptp_on - 0.20, ptp_off * 1.50, 0.50).` Include the actual numbers in the comment for traceability.

    Sanity bounds (if outside, flag for human review):
    - If `ptp_on < 0.70`: WARN — Lever 2 is failing to preserve amplitude at M+30. Possible bug in `_split_level_anomaly` or `assembler` integration. Stop the plan and request investigation.
    - If `ptp_on > 1.10`: WARN — anomaly is producing un-bounded growth. Possible level computation error. Stop.
    - If `ptp_off > 0.70`: WARN — legacy M+30 amplitude is much higher than RESEARCH analytic estimate 0.52. Could indicate the bowl fixture is too aggressive; verify against fixture sanity. Continue but document.

    If the bounds hold, commit and proceed to Task 4. DO NOT touch any test body in this task — only the constant.
  </action>
  <verify>
    <automated>python -c "import re; src = open('tests/test_shape_hourly_bowl.py').read(); m = re.search(r'SC3_M30_AMPLITUDE_THRESHOLD\s*=\s*([0-9.]+)', src); assert m, 'constant not found'; val = float(m.group(1)); assert 0.50 <= val <= 1.00, f'threshold {val} out of plausible bounds [0.50, 1.00]'; assert 'Wave 0 calibrated' in src and 'Plan 05C-02' in src, 'traceability comment missing'; print(f'OK threshold={val:.4f}')"</automated>
  </verify>
  <acceptance_criteria>
    - `grep -q "SC3_M30_AMPLITUDE_THRESHOLD =" tests/test_shape_hourly_bowl.py` exits 0.
    - `grep -q "Wave 0 calibrated" tests/test_shape_hourly_bowl.py` exits 0.
    - `grep -q "Plan 05C-02" tests/test_shape_hourly_bowl.py` exits 0.
    - The verify command above prints `OK threshold=<value>` and exits 0.
    - The measured `threshold` value is in the bounds `[0.50, 1.00]`.
    - Original placeholder comment `# TODO Plan 05C-02 Wave 0` is REMOVED from the file (no leftover TODO).
    - `pytest tests/ -x -q` exits 0 reporting `249 passed, 4 skipped` (no test references the threshold yet — Task 4 adds the consumer).
  </acceptance_criteria>
  <done>Threshold calibrated and committed with traceability comment. The number is now the contract for `test_f_H_amplitude_preserved_at_M30` (Task 4).</done>
</task>
...
<verification>
- `pytest tests/ -x -q` exits 0 reporting `251 passed, 4 skipped`.
- `pytest tests/ --co -q | tail -1` reports `>= 257 tests collected`.
...
- Test count: 249 to 251 (4 skipped preserved).
