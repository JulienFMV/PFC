...
Follow-up implementation note: the existing locked T057 plan JSON was not
rewritten, preserving its registered hash. Future plans generated by
`scripts/plan_epex_lab_locked_holdout.py` now include
`commands.run_locked_holdout_template`, pointing to
`scripts/run_epex_lab_locked_holdout.py`, so new plans advertise the
fail-closed wrapper as the preferred execution path while still retaining the
separate backtest and audit command templates for traceability.
...
## D-20260709-58 - Clarify Phase 14 Context Supersession And T057 Outcome Policy

Decision: `.planning/CONTEXT.md` is historical Phase 5bis context for the
current Phase 14 workstream and must not be used as the active handoff. The
2026-06-18 residual-anchor external audit prompt is also historical diagnostic
context, not the target architecture. The active Phase 14 architecture remains
the 2026-06-19 monthly solver reform and its later T056/T057 governance chain.

Reason: a read-only expert audit found that agents could restart from
`.planning/CONTEXT.md` or the 2026-06-18 prompt and pursue an obsolete
residual-anchor path. That would conflict with the accepted monthly solver
reform, T056/T057 freeze discipline, and no-patch-after-solver invariant.

Implementation:

- Added a top-level notice to `.planning/CONTEXT.md` pointing agents to
  `AGENTS.md`, `.planning/HANDOFF.md`, the Phase 14 decision log, and the
  latest session handoff.
- Recorded that the 2026-06-18 residual-anchor prompt is superseded by the
  2026-06-19 monthly solver reform for Phase 14 architecture decisions.
- Clarified T057 outcome policy:
  - `LOCKED_HOLDOUT_PASS` permits only the next production packaging step;
  - it is not automatic production promotion;
  - `NO_GO` or failed holdout requires a new pre-registered lineage, not
    retuning T056 against the locked window.
- Clarified that T057 `min_residual_mae_improvement_eur_mwh >= 0.0` is a
  non-degradation gate, not a material superiority threshold.

Rejected alternatives:

- Rewrite historical planning documents in place and risk losing context.
- Treat T057 PASS as final production approval.
- Tune T056/T057 parameters after seeing locked-window outcomes.

Invariants not to break:

- Monthly solver remains the level authority.
- External information, including EPEX and neighbors, may enter only as
  governed shape; OMPEX remains advisory-only and never an input, target, loss,
  selection criterion, or gate.
- T057 plan JSON remains locked; new needs require a new plan id.
...
## D-20260709-99 - Adjusted Production Chain Must Rebind Embedded Holdout Policy

Decision: `build_epex_lab_adjusted_production_chain.py` must reject an approved
adjusted production manifest when its embedded `locked_holdout_policy` does not
match the policy recomputed from the hash-bound locked holdout summary.

Reason: the chain builder already verified that the referenced locked holdout
summary exists, is hash-bound, and passes policy. However, it copied the
production manifest's embedded `locked_holdout_policy` into export/selected/
capstone artifacts without proving that embedded object was current. A stale or
hand-edited manifest could therefore carry contradictory audit metadata even
though the referenced holdout itself passed.

Implementation:

- `scripts/build_epex_lab_adjusted_production_chain.py` recomputes
  `locked_holdout_policy` from the referenced summary.
- The chain now raises `locked_holdout_policy_bound` when the embedded policy
  differs from the recomputed policy.
- Added a regression test that tampers only the embedded policy status while
  leaving the holdout summary hash-bound.
...
- Trust `locked_holdout_policy_pass=true` plus the linked holdout summary only.
- Drop embedded `locked_holdout_policy` from downstream artifacts.

Invariants:

- This does not change the locked holdout pass criteria.
- Manifests generated by `build_epex_lab_adjusted_production_manifest.py`
  remain valid when their linked holdout policy passes.
