...
- Production monthly manifest producer
  - `save_long_term_outputs(...)` now writes
    `pfc_shaping/model/artifacts/production_monthly_curve_manifest.json` when
    the CH monthly solver is active.
  - The file contains the manifest generated by the production
    `monthly_curve_authority` call, including `monthly_solution_hash`,
    `active_constraints_hash` and `active_config_hash`.
...
pytest tests/test_long_term_branch.py tests/test_monthly_forward_curve_integration.py tests/test_check_monthly_curve_promotion_from_manifests.py -q
...
- Production manifest writer targeted tests:
  `26 passed`
- Monthly family plus LT/CT import guard after production manifest writer:
...
- `lambda_calibration_artifact_present` and `production_export_path_parity`
  are emitted by the sparse proof when requested. Full candidate approval must
  use the manifest-backed capstone script with a real production manifest from
  `pfc_shaping/model/artifacts/production_monthly_curve_manifest.json`, a real
  export manifest, and the selected lambda artifact.
