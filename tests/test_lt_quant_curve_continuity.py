...
def _monthly_constraint_system(index: pd.DatetimeIndex, targets: dict[int, float]) -> ConstraintSystem:
    rows = []
    for month, target in targets.items():
        mask = (index.month == month).astype(float)
        rows.append(
            ConstraintRow(
...
def test_optimizer_smoothness_penalties_remove_flat_delta_month_cliffs() -> None:
    ...
    assert max(_month_boundary_jumps(smooth.curve)) < 0.1
