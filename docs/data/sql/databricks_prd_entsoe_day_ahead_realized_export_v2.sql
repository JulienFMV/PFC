-- Consumer-complete, one-month ENTSO-E day-ahead realized-truth candidate.
-- Latest observed revision is not itself finality proof; local admission also
-- requires separate hash-bound finality evidence.  A result with 20,001 rows
-- is a rejection sentinel, not a usable extract.
WITH parameters AS (
  SELECT
...
eligible AS (
  SELECT
    ...
    v.source_file_path,
    v.last_seen_publication_timestamp_utc,
  ...
  WHERE v.group_name = 'day_ahead_prices'
    ...
    AND v.first_seen_pull_ts_utc IS NOT NULL
    AND v.first_seen_pull_ts_utc <= p.assessed_at_utc
    AND COALESCE(v.last_seen_pull_ts_utc, v.first_seen_pull_ts_utc)
      <= p.assessed_at_utc
    ...
    *,
    ROW_NUMBER() OVER (
      ...
      ORDER BY
        ...
        last_seen_publication_timestamp_utc DESC NULLS LAST,
